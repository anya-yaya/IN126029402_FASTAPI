...
#------ 1. QUERY parameter- Filter Products by minimum price
...
#----- 4. GET + QUERY +BUSINESS LOGIC- Build a Product Summary dashboard
...
#------ 2. GET + PATH PARAMETER- Get only price of a product 
...
'total_price':total,'status':'pending'}     #change status to pending from confirmed
    orders.append(order)
    order_counter += 1
...
#----- ⭐Bonus: POST + GET + ORDER TRACKING- Order Status Tracker
# PATCH to confirm
...
#----- 3. PYDANTIC + POST- Accept customer feedback
@app.post('/feedback')
...
#----- 5. POST + PYDANTIC +BUSINESS LOGIC- Validate and Place a Bulk Order
@app.post('/orders/bulk')
def place_bulk_order(order: BulkOrderRequest):
